...
export async function POST(request: Request) {
  ...
  try {
    ...
    const filename = `cleanup-sound.${ext}`;
    const dataDir = getDataDir();
    const filePath = path.join(dataDir, filename);

    // Remove old sound if exists
    const oldSetting = await prisma.setting.findUnique({ where: { key: "cleanup_sound_path" } });
    if (oldSetting?.value) {
    ...
    await prisma.setting.upsert({
      where: { key: "cleanup_sound_path" },
      update: { value: filename },
      create: { key: "cleanup_sound_path", value: filename },
    });
...
export async function DELETE() {
  ...
  try {
    const setting = await prisma.setting.findUnique({ where: { key: "cleanup_sound_path" } });
    if (setting?.value) {
      const dataDir = getDataDir();
    ...
    await prisma.setting.upsert({
      where: { key: "cleanup_sound_path" },
      update: { value: "" },
      create: { key: "cleanup_sound_path", value: "" },
    });
